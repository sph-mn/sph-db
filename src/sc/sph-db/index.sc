...
(define (db-index-system-key type-id fields fields-len result-data result-size)
  ...
  (status-require (sph-memory-malloc size (convert-type &data void**)))
  (set
...
(define (db-index-name type-id fields fields-len result result-len)
  (status-t db-type-id-t db-fields-len-t* db-fields-len-t char** size-t*)
  ...
  (declare
    ...
    strings-len int
    name char*)
  (define prefix char* "i")
  (set strings 0 strings-len (+ 2 fields-len))
  (status-require (sph-memory-calloc (* strings-len (sizeof char*)) (convert-type &strings void**)))
  (sc-comment "type id")
...
(define (db-index-key env index values result-data result-size)
  ...
  (status-require (sph-memory-calloc size &data))
  (set data-temp data)
  (for ((set i 0) (< i index.fields-len) (set i (+ 1 i)))
    (set
      field-index (array-get index.fields i)
...
(define (db-indices-entry-ensure txn values id) (status-t db-txn-t db-record-values-t db-id-t)
  ...
  (label exit (db-mdb-cursor-close-if-active record-index-cursor) (free data) status-return))
...
(define (db-type-indices-add type index result) (status-t db-type-t* db-index-t db-index-t**)
  ...
  (status-require
    (sph-memory-realloc (* indices-len (sizeof db-index-t)) (convert-type &indices void**)))
  (set
...
(define (db-index-create env type fields fields-len result-index)
  ...
  (declare
    ...
    record-index db-index-t)
  (if (not fields-len) (begin (set status.id db-status-id-invalid-argument) status-return))
  ...
  (status-require
    (sph-memory-malloc (* fields-len (sizeof db-fields-len-t)) (convert-type &fields-copy void**)))
...
(define (db-index-delete env index) (status-t db-env-t* db-index-t*)
  ...
  (label exit (db-mdb-cursor-close-if-active system) (db-txn-abort-if-active txn) status-return))

(define (db-index-rebuild env index) (status-t db-env-t* db-index-t*)
  "clear index and fill with data from existing records"
  status-declare
  (db-txn-declare env txn)
  (declare name char* name-len size-t)
  (set name 0)
  ...
  (label exit (free name) (db-txn-abort-if-active txn) status-return))
...
(define (db-index-read selection count result-ids)
  ...
  (do-while count
    (sph-array-add *result-ids (db-pointer->id val-id.mv-data))
    (set count (- count 1))
