(define (db-record-values->data values result) (status-t db-record-values-t db-record-t*)
  ...
  (if (not values.extent) (begin (set result:data 0 result:size 0) status-return))
  ...
  (status-require (sph-memory-calloc size (convert-type &data void**)))
  (set data-temp data)
  (for ((set i 0) (< i values.extent) (set i (+ 1 i)))
    (set
...
(define (db-record-values-new type result) (status-t db-type-t* db-record-values-t*)
  ...
  (status-require
    (sph-memory-calloc (* type:fields-len (sizeof db-record-value-t)) (convert-type &data void**)))
...
(define (db-record-create txn values result) (status-t db-txn-t db-record-values-t db-id-t*)
  ...
  (set *result id)
  (label exit (db-mdb-cursor-close-if-active records) (free record.data) status-return))
...
(define (db-record-data->values type data result)
  ...
  (set *result values)
  (label exit (if status-is-failure (db-free-record-values &values)) status-return))
        ...
        (if (not skip)
          (begin
            (set
              ...
              record.data val-data.mv-data
              record.size val-data.mv-size)
            (sph-array-add *result-records record)))
        (set count (- count 1))))
...
(define (db-record-get txn ids match-all result-records)
  ...
  (label exit (db-mdb-cursor-close records) status-return))

(define (db-relation-internal-delete left right label ordinal relation-lr relation-rl relation-ll)
  (status-t db-ids-t* db-ids-t* db-ids-t* db-ordinal-condition-t* MDB-cursor* MDB-cursor* MDB-cursor*)
...
(define (db-record-delete txn ids) (status-t db-txn-t db-ids-t)
  ...
  (for-each-index i size-t
    ...
    (if db-mdb-status-is-success
      (begin
        (set id (sph-array-get ids i) record.data val-data.mv-data record.size val-data.mv-size)
        (status-require
...
(define (db-record-index-select txn index values result-selection)
  ...
  (label exit (if status-is-failure (db-mdb-cursor-close-if-active records)) status-return))
