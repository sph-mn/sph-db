...
#include <sph-db/sph/set.h>
#include <sph-db/config.h>
typedef struct {
  db_id_t left;
  db_id_t right;
  db_id_t label;
...
#define sph_array_current_declare_struct_type(name, element_type) \
  typedef struct { \
    size_t size; \
    size_t used; \
    element_type* data; \
...
#define sph_array_current_declare_type(name, element_type) sph_array_declare_type_custom(name, element_type, sph_array_default_alloc, sph_array_default_realloc, free, sph_array_current_declare_struct_type)
#define sph_array_current_set_null(a) \
  sph_array_set_null(a); \
  a.current = 0
#define sph_array_current_forward(a) a.current += 1
#define sph_array_current_rewind(a) a.current = 0
#define sph_array_current_in_range(a) (a.current < a.used)
#define sph_array_current_get(a) sph_array_get(a, (a.current))
sph_array_current_declare_type(db_ids, db_id_t)
  sph_array_current_declare_type(db_records, db_record_t)
    sph_array_current_declare_type(db_relations, db_relation_t)
      sph_set_declare_type(db_id_set, db_id_t, sph_set_hash_integer, sph_set_equal_integer, 0, 2)
...
#define db_ids_add sph_array_add
#define db_ids_clear sph_array_clear
#define db_ids_forward sph_array_current_forward
#define db_ids_get sph_array_current_get
#define db_ids_get_at sph_array_current_get_at
...
#define db_relations_clear sph_array_clear
#define db_relations_forward sph_array_current_forward
#define db_relations_get sph_array_current_get
#define db_relations_get_at sph_array_current_get_at
...
#define db_records_clear sph_array_clear
#define db_records_forward sph_array_current_forward
#define db_records_get sph_array_current_get
#define db_records_get_at sph_array_current_get_at
...
#define db_ids_declare(name) sph_array_declare(name, db_ids_t)
#define db_relations_declare(name) sph_array_declare(name, db_relations_t)
#define db_records_declare(name) sph_array_declare(name, db_records_t)
...
struct db_relation_selection_t;
typedef status_t (*db_relation_reader_t)(struct db_relation_selection_t*, db_count_t, db_relations_t*);
typedef struct db_relation_selection_t {
...
status_t db_open(char* root, db_open_options_t* options, db_env_t* env);
db_field_t* db_type_field_get(db_type_t* type, char* name);
db_type_t* db_type_get(db_env_t* env, char* name);
status_t db_type_create(db_env_t* env, char* name, db_field_t* fields, db_fields_len_t fields_len, uint8_t flags, db_type_t** result);
status_t db_type_delete(db_env_t* env, db_type_id_t id);
uint8_t db_field_type_size(db_field_type_t a);
char* db_status_description(status_t a);
char* db_status_name(status_t a);
