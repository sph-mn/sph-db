...
status_t db_relation_ensure(db_txn_t txn, db_ids_t left, db_ids_t right, db_ids_t label, db_relation_ordinal_generator_t ordinal_generator, void* ordinal_generator_state) {
  ...
  ordinal = ((!ordinal_generator && ordinal_generator_state) ? *((db_ordinal_t*)(ordinal_generator_state)) : 0);
  while (sph_array_current_in_range(left)) {
    ...
    while (sph_array_current_in_range(label)) {
      ...
      while (sph_array_current_in_range(right)) {
...
status_t db_relation_index_rebuild(db_env_t* env) {
  ...
  db_mdb_status_require((db_mdb_env_cursor_open(txn, relation_rl)));
  db_mdb_status_require((db_mdb_env_cursor_open(txn, relation_ll)));
  status.id = mdb_cursor_get(relation_lr, (&val_relation_key), (&val_relation_data), MDB_FIRST);
  while (db_mdb_status_is_success) {
    id_left = db_pointer_to_id_at((val_relation_key.mv_data), 0);
    id_label = db_pointer_to_id_at((val_relation_key.mv_data), 1);
    do {
      id_right = db_pointer_to_id((val_relation_data.mv_data));
      /* relation-rl */
      relation_key[0] = id_right;
      relation_key[1] = id_label;
      val_relation_key.mv_data = relation_key;
