...
status_t db_open_root(db_env_t* env, db_open_options_t* options, char* path) {
  status_declare;
  char* path_temp;
  path_temp = 0;
...
status_t db_open_format(MDB_cursor* system, db_txn_t txn) {
  ...
  val_key.mv_size = sizeof(uint8_t);
  val_data.mv_size = sizeof(format_data);
...
status_t db_open_system_sequence(MDB_cursor* system, db_type_id_t* result) {
  ...
  if (db_mdb_status_is_success) {
    if (db_system_label_type == db_system_key_label((val_key.mv_data))) {
      ...
      if (db_mdb_status_is_success && (db_system_label_type == db_system_key_label((val_key.mv_data)))) {
        current = db_system_key_id((val_key.mv_data));
        goto exit;
      } else {
        db_mdb_status_expect_notfound;
...
status_t db_open_type_read_fields(uint8_t** data_pointer, db_type_t* type) {
  ...
  status_require((sph_memory_calloc((count * sizeof(db_field_t)), ((void**)(&fields)))));
  for (i = 0; (i < count); i += 1) {
    field_type = *data;
  ...
  if (fixed_count) {
    status_require((sph_memory_malloc(((1 + fixed_count) * sizeof(size_t)), ((void**)(&fixed_offsets)))));
    for (i = 0; (i < fixed_count); i = (1 + i)) {
      fixed_offsets[i] = offset;
...
status_t db_open_type(uint8_t* system_key, uint8_t* system_value, db_type_t* types, MDB_cursor* records, db_type_t** result_type) {
  ...
  type_pointer->indices = 0;
  type_pointer->indices_len = 0;
  type_pointer->id = id;
...
status_t db_open_types(MDB_cursor* system, MDB_cursor* records, db_txn_t txn) {
  ...
  val_key.mv_size = db_size_system_key;
  val_key.mv_data = key;
  status_require((sph_memory_malloc((types_len * sizeof(db_type_t)), ((void**)(&types)))));
  /* calloc doesnt necessarily set struct fields to zero */
  for (i = 0; (i < types_len); i = (1 + i)) {
    (types[i]).id = 0;
  };
  (types[0]).sequence = system_sequence;
  status.id = mdb_cursor_get(system, (&val_key), (&val_data), MDB_SET_RANGE);
  /* record type sequences */
  while ((db_mdb_status_is_success && (db_system_label_type == db_system_key_label((val_key.mv_data))))) {
...
status_t db_open_indices(MDB_cursor* system, db_txn_t txn) {
  ...
  while ((db_mdb_status_is_success && (db_system_label_index == db_system_key_label((val_key.mv_data))))) {
    ...
    if (current_type_id == type_id) {
      /* another index for the same type */
      indices_len = (1 + indices_len);
      if (indices_len > indices_alloc_len) {
        indices_alloc_len = (10 + indices_alloc_len);
        status_require((sph_memory_realloc((indices_alloc_len * sizeof(db_index_t)), ((void**)(&indices)))));
      };
    } else {
      ...
      if (indices_len) {
        ...
        if (indices_alloc_len > indices_len) {
          status_require((sph_memory_realloc((indices_len * sizeof(db_index_t)), ((void**)(&indices)))));
        };
        /* add indices array to type */
        (types[current_type_id]).indices = indices;
      };
      /* set current type and allocate indices array */
      current_type_id = type_id;
      indices_len = 1;
      indices_alloc_len = 10;
      status_require((sph_memory_calloc((indices_alloc_len * sizeof(db_index_t)), ((void**)(&indices)))));
    };
    fields_len = ((val_key.mv_size - db_size_system_key) / sizeof(db_fields_len_t));
    status_require((sph_memory_calloc((fields_len * sizeof(db_fields_len_t)), ((void**)(&fields)))));
...
status_t db_open(char* path, db_open_options_t* options_pointer, db_env_t* env) {
  status_declare;
