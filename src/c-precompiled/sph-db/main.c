...
void db_debug_log_id_bits(db_id_t a) {
  db_id_t index;
  printf(db_id_printf_format, (1 & a));
  for (index = 1; (index < (8 * sizeof(db_id_t))); index += 1) {
    printf("%" PRIu8, (((((db_id_t)(1)) << index) & a) ? 1 : 0));
  };
  printf("\n");
}
...
void db_debug_log_ids(db_ids_t a) {
  ...
  while (sph_array_current_in_range(a)) {
    printf("  " db_id_printf_format, (sph_array_current_get(a)));
...
void db_debug_log_ids_set(db_id_set_t a) {
  ...
  for (uint32_t i = 0; (i < a.size); i += 1) {
    if ((a.values)[i]) {
      printf("  " db_id_printf_format, ((a.values)[i]));
    };
  };
  printf("\n");
}
void db_debug_log_relations(db_relations_t a) {
  ...
  while (sph_array_current_in_range(a)) {
    b = sph_array_current_get(a);
    printf(("  " db_id_printf_format db_id_printf_format " -> " db_ordinal_printf_format db_id_printf_format "\n"), (b.left), (b.label), (b.ordinal), (b.right));
    sph_array_current_forward(a);
  };
}
...
status_t db_debug_count_all_btree_entries(db_txn_t txn, size_t* result) {
  status_declare;
  db_statistics_t stat;
  status_require((db_statistics(txn, (&stat))));
...
status_t db_ids_to_set(db_ids_t a, db_id_set_t* result) {
  ...
  while (sph_array_current_in_range(a)) {
    db_id_set_add((&b), (sph_array_current_get(a)));
    sph_array_current_forward(a);
  };
  *result = b;
exit:
...
status_t db_read_name(uint8_t** data_pointer, char** result) {
  status_declare;
  uint8_t* data;
  db_name_len_t len;
  char* name;
  ...
  status_require((sph_memory_malloc_string(len, (&name))));
...
void db_records_to_ids(db_records_t records, db_ids_t* result_ids) {
  while (sph_array_current_in_range(records)) {
    sph_array_add((*result_ids), ((sph_array_current_get(records)).id));
    sph_array_current_forward(records);
  };
}
...
status_t db_env_new(db_env_t** result) {
  status_declare;
  db_env_t* a;
  status_require((sph_memory_calloc((sizeof(db_env_t)), ((void**)(&a)))));
  *result = a;
exit:
  status_return;
}
