...
status_t test_type_create_many(db_env_t* env) {
  ...
  for (i = 0; (i < (10 * db_env_types_extra_count)); i += 1) {
    sprintf(name, "test-type-" db_type_id_printf_format, i);
...
status_t test_record_create(db_env_t* env) {
  status_declare;
  db_txn_declare(env, txn);
  sph_array_declare(ids, db_ids_t);
  sph_array_declare(records, db_records_t);
  ...
  sph_array_add(ids, id_1);
  sph_array_add(ids, id_2);
  status_require((db_record_get(txn, ids, 1, (&records))));
  test_helper_assert("record-get result length", (2 == records.used));
  test_helper_assert("record-get result ids", ((id_1 == (sph_array_get(records, 0)).id) && (id_2 == (sph_array_get(records, 1)).id)));
  field_data = db_record_ref(type, (sph_array_get(records, 0)), 1);
  test_helper_assert("record-ref-2", ((2 == field_data.size) && (value_2 == *((int8_t*)(field_data.data)))));
  field_data = db_record_ref(type, (sph_array_get(records, 0)), 3);
  test_helper_assert("record-ref-3", ((5 == field_data.size) && (0 == memcmp(value_4, (field_data.data), (field_data.size)))));
  sph_array_clear(ids);
  sph_array_clear(records);
  sph_array_add(ids, 9999);
...
status_t test_record_select(db_env_t* env) {
  ...
  size_t btree_size_before_delete;
  size_t btree_size_after_delete;
  db_type_t* type;
  ...
  test_helper_assert("record-read size", (1 == records.used));
  record_value = db_record_ref(type, (sph_array_current_get(records)), 0);
  ...
  test_helper_assert("current id set", (db_id_element(((sph_array_current_get(records)).id))));
  status_require((db_record_read(selection, 1, (&records))));
  status_require((db_record_read(selection, 1, (&records))));
  status = db_record_read(selection, 1, (&records));
  test_helper_assert("all type entries found", ((db_status_id_notfound == status.id) && (4 == records.used)));
  status.id = status_id_success;
  ...
  sph_array_add(ids, (record_ids[0]));
  sph_array_add(ids, (record_ids[2]));
...
status_t test_index(db_env_t* env) {
  ...
  db_id_t* record_ids;
  uint32_t record_ids_len;
  char* index_name_expected = "i-1-1-2";
  fields_len = 2;
  status_require((test_helper_create_type_1(env, (&type))));
  status_require((test_helper_create_values_1(env, type, (&values), (&values_len))));
  ...
  test_helper_assert("key memory ref", (((char*)(key_data))[3]));
  ...
  test_helper_assert("index-read ids length", (2 == ids.used));
  test_helper_assert("index-select type-id 1", (type->id == db_id_type((sph_array_get(ids, 0)))));
  test_helper_assert("index-select type-id 2", (type->id == db_id_type((sph_array_get(ids, 1)))));
  ...
  test_helper_assert("index-select type-id 1", ((1 == ids.used) && (type->id == db_id_type((sph_array_current_get(ids))))));
  db_txn_abort((&txn));
  status_require((db_txn_begin((&txn))));
