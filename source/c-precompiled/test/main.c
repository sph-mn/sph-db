...
status_t test_node_select(db_env_t* env) {
  ...
  test_helper_assert(
    "all type entries found", (db_status_id_notfound == status.id));
  status.id = status_id_success;
  ...
  /* matcher */
  matcher_state = 0;
  status_require(db_node_select(
    txn, 0, type, 0, node_matcher, (&matcher_state), (&selection)));
  status_require(db_node_next((&selection)));
  data = db_node_ref((&selection), 0);
  test_helper_assert("node-ref size", (1 == data.size));
...
status_t
test_helper_dbi_entry_count(db_txn_t txn, MDB_dbi dbi, size_t* result) {
  status_declare;
  MDB_stat stat;
