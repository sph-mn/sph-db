...
status_t db_graph_read_1000(db_graph_read_state_t* state,
  ...
  if (db_id_equal_p(db_pointer_to_id(val_graph_key.mv_data, 0), graph_key[0])) {
    goto each_data;
  } else {
  set_range:
...
status_t db_graph_read_1100(db_graph_read_state_t* state,
  ...
  db_mdb_cursor_get_norequire(graph_rl, val_graph_key, val_id, MDB_GET_CURRENT);
  db_mdb_status_require;
...
status_t db_graph_read_1001_1101(db_graph_read_state_t* state,
  ...
  if ((db_id_equal_p(
         db_pointer_to_id(val_graph_key.mv_data, 0), graph_key[0]) &&
        ((!ordinal_min ||
          ((db_graph_data_to_ordinal(val_graph_data.mv_data) >=
            ordinal_min)))) &&
        ((!ordinal_max ||
          ((db_graph_data_to_ordinal(val_graph_data.mv_data) <=
            ordinal_max)))))) {
...
status_t db_graph_read_0010(db_graph_read_state_t* state,
  ...
  db_id_t id_left;
  db_id_t id_label;
  db_mdb_cursor_get_norequire(graph_ll, val_id, val_id_2, MDB_GET_CURRENT);
  db_mdb_status_require;
  db_mdb_cursor_get_norequire(
    graph_lr, val_graph_key, val_graph_data, MDB_GET_CURRENT);
  db_mdb_status_require;
...
status_t db_graph_read_0100(db_graph_read_state_t* state,
  ...
  db_mdb_status_require;
  graph_key[0] = db_ids_first(right);
  if (db_id_equal_p(graph_key[0], db_pointer_to_id(val_graph_key.mv_data, 0))) {
    goto each_key;
  } else {
  set_range:
    val_graph_key.mv_data = graph_key;
