...
#define db_mdb_val_to_id_at(a, index) db_pointer_to_id(a.mv_data, index)
...
static int db_mdb_compare_id(const MDB_val* a, const MDB_val* b) {
  return (db_id_compare(db_pointer_to_id(db_mdb_compare_get_mv_data(a), 0),
    db_pointer_to_id(db_mdb_compare_get_mv_data(b), 0)));
};
static int db_mdb_compare_graph_key(const MDB_val* a, const MDB_val* b) {
  return (((db_mdb_val_to_id_at((*a), 0) < db_mdb_val_to_id_at((*b), 0))
      ? -1
      : ((db_mdb_val_to_id_at((*a), 0) > db_mdb_val_to_id_at((*b), 0))
            ? 1
            : ((db_mdb_val_to_id_at((*a), 1) < db_mdb_val_to_id_at((*b), 1))
                  ? -1
                  : (db_mdb_val_to_id_at((*a), 1) >
                      db_mdb_val_to_id_at((*b), 1))))));
};
/** memcmp does not work here, gives -1 for 256 vs 1 */
static int db_mdb_compare_graph_data(const MDB_val* a, const MDB_val* b) {
  return (((db_mdb_val_graph_data_to_ordinal((*a)) <
             db_mdb_val_graph_data_to_ordinal((*b)))
      ? -1
      : ((db_mdb_val_graph_data_to_ordinal((*a)) >
           db_mdb_val_graph_data_to_ordinal((*b)))
            ? 1
            : ((db_mdb_val_graph_data_to_id((*a)) <
                 db_mdb_val_graph_data_to_id((*b)))
                  ? -1
                  : (db_mdb_val_graph_data_to_id((*a)) >
                      db_mdb_val_graph_data_to_id((*b)))))));
};
