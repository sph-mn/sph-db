...
(define (imht-set-create min-size result) (uint8-t size-t imht-set-t**)
  ...
  (struct-set **result content (calloc min-size (sizeof imht-set-key-t)) size min-size)
  (return (if* (: *result content) 0 1)))

(define (imht-set-destroy a) (void imht-set-t*) (if a (begin (free a:content) (free a))))
  ...
  (if *h
    (begin
      ...
      (while (< h2 content-end)
        ...
        (if (not *h2) (return 0) (if (= value *h2) (return h2)))
        (set h2 (+ 1 h2)))
      (if (not *h2) (return 0) (if (= value *h2) (return h2)))
      (set h2 a:content)
      (while (< h2 h) (if (not *h2) (return 0) (if (= value *h2) (return h2))) (set h2 (+ 1 h2)))))
  (return 0))
...
(define (imht-set-remove a value) (uint8-t imht-set-t* imht-set-key-t)
  ...
  (if value-address (begin (set *value-address 0) (return #t)) (return #f)))
...
(define (imht-set-add a value) (imht-set-key-t* imht-set-t* imht-set-key-t)
  ...
  (if *h
    (begin
      ...
      (pre-if imht-set-can-contain-zero (if (or (= value *h) (= 0 value)) (return h))
        (if (= value *h) (return h)))
      (define content-end imht-set-key-t* (+ a:content (- a:size 1)))
      (define h2 imht-set-key-t* (+ 1 h))
      (while (and (<= h2 content-end) *h2) (set h2 (+ 1 h2)))
      ; has been moved to the next entry without data or past the last entry
      (if (> h2 content-end)
        (begin
          (set h2 a:content)
          (while (and (< h2 h) *h2) (set h2 (+ 1 h2)))
          (if (= h2 h) (return 0)
            (pre-if imht-set-can-contain-zero (set *h2 (if* (= 0 value) 1 value)) (set *h2 value))))
        (pre-if imht-set-can-contain-zero (set *h2 (if* (= 0 value) 1 value)) (set *h2 value))))
    (begin
      (pre-if imht-set-can-contain-zero (set *h (if* (= 0 value) 1 value)) (set *h value))
      (return h))))