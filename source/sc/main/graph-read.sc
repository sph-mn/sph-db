  ...
  (if (db-id-equal? (db-pointer->id val-graph-key.mv-data 0) (array-get graph-key 0))
    ...
    (label set-range
      ...
      (label each-right
        ...
        (if right (array-set-index graph-key 0 (db-ids-first right))
          no-more-data-exit)
        (goto set-range))))
  (label each-left
    stop-if-count-zero
...
(define (db-graph-read-1110 state count result)
  ...
  (label set-cursor
    ...
    (db-mdb-cursor-get-norequire graph-rl val-graph-key val-id MDB-GET-BOTH)
    (if db-mdb-status-success? (goto match)
      db-mdb-status-require-notfound)
    (label next-query
      (set right (db-ids-rest right))
      (if right
...
(define (db-graph-read-1001-1101 state count result)
  ...
  (db-mdb-cursor-get-norequire graph-lr val-graph-key val-graph-data MDB-GET-CURRENT)
  db-mdb-status-require
  (if left (array-set-index graph-key 0 (db-ids-first left))
    no-more-data-exit)
  (if
    (and
...
(define (db-graph-read-0010 state count result)
  ...
  (label each-label-data
    ...
    (if db-mdb-status-success? (goto each-label-data)
      (begin
        (set label (db-ids-rest label))
        (if label (set id-label (db-ids-first label))
          no-more-data-exit)
  ...
  (label each-key
    (label each-data
      ...
      (if db-mdb-status-success? (goto each-data)
        db-mdb-status-require-notfound))
    (db-mdb-cursor-next-nodup-norequire graph-lr val-graph-key val-graph-data)
    (if db-mdb-status-success? (goto each-key)
      db-mdb-status-require-notfound))
  (label exit
